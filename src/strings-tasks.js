...
function countVowels(str) {
  const a = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  let s = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (a.includes(str[i])) {
      s += 1;
...
function isPalindrome(str) {
  let normalizeStr = str.replaceAll(' ', '').replaceAll(',', '');

  if (normalizeStr.includes('!') || normalizeStr.includes('?')) {
    normalizeStr = normalizeStr.slice(0, normalizeStr.length - 1);
  }
  const p = normalizeStr.split('').reverse().join('');
  return normalizeStr.toLowerCase() === p.toLowerCase();
}
...
function reverseWords(str) {
  const arr = str.split(' ').map((index) => index.split('').reverse().join(''));
  return arr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const alRot = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm?!';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    const index = al.indexOf(str[i]);
    if (index === -1) {
      result += ' ';
    } else {
      result += alRot[index];
...
function getCardId(value) {
  ...
  const valueIndex = values.indexOf(value.slice(0, -1));
